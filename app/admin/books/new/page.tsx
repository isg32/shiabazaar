...
export default function NewBookPage() {
  ...
  const [form, setForm] = useState({
    title: "", slug: "", price: "", originalPrice: "", inStock: true, badge: "",
    author: "", isbn: "", publisher: "", language: "", genre: "",
    pageCount: "", edition: "", description: "", tableOfContents: "", categoryId: "",
  });
  ...
  useEffect(() => {
    fetch("/api/admin/categories").then(r => r.json())
      .then(d => setAllCategories((d.categories ?? []).filter((c: NavCategory) => c.group === "book")))
      .catch(() => {});
  }, []);
  ...
  async function uploadToCloudinary(file: File, slug: string) {
    ...
    fd.append("file", file); fd.append("timestamp", sign.timestamp);
    fd.append("signature", sign.signature); fd.append("api_key", sign.apiKey);
    fd.append("folder", sign.folder);
    const data = await fetch(`https://api.cloudinary.com/v1_1/${sign.cloudName}/image/upload`, { method: "POST", body: fd }).then(r => r.json());
  ...
  async function createCategory() {
    ...
    try {
      const res = await fetch("/api/admin/categories", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: newCatName.trim(), slug: slugify(newCatName), group: "book" }),
      });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      for (const p of previews) {
        const { url, cloudinaryId } = await uploadToCloudinary(p.file, product.slug);
  ...
  return (
    <div className="px-8 py-8 text-on-dark max-w-3xl">
      <div className="mb-6">
        <h1 className="text-2xl font-normal text-on-dark" style={{ fontFamily: "var(--font-display)" }}>Add Book</h1>
        <p className="text-sm text-on-dark-soft mt-0.5">New book will appear on the storefront immediately.</p>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className={sectionCls}>
          <h2 className="text-sm font-medium text-on-dark mb-5">Basic Information</h2>
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2">
              <label className={labelCls}>Title *</label>
              <input className={inputCls} value={form.title} onChange={e => set("title", e.target.value)} placeholder="Nahjul Balagha" required />
            </div>
            <div>
              <label className={labelCls}>Slug *</label>
              <input className={inputCls} value={form.slug} onChange={e => set("slug", slugify(e.target.value))} placeholder="nahjul-balagha" required />
            </div>
            <div>
            ...
            <div>
              ...
              <input className={inputCls} type="number" min="0" step="0.01" value={form.price} onChange={e => set("price", e.target.value)} placeholder="699" required />
            </div>
            <div>
              <label className={labelCls}>Original Price (₹)</label>
              <input className={inputCls} type="number" min="0" step="0.01" value={form.originalPrice} onChange={e => set("originalPrice", e.target.value)} placeholder="899" />
            </div>
            <div>
              <label className={labelCls}>Badge</label>
              <select className={inputCls} value={form.badge} onChange={e => set("badge", e.target.value)}>
                <option value="">None</option>
                <option>NEW</option><option>BESTSELLER</option><option>FEATURED</option><option>ON SALE</option>
              </select>
            </div>
            ...
            <div className="sm:col-span-2">
              ...
              <div className="flex gap-2">
                <input className={`${inputCls} flex-1`} value={newCatName} onChange={e => setNewCatName(e.target.value)}
                  placeholder="New category name…" onKeyDown={e => { if (e.key === "Enter") { e.preventDefault(); createCategory(); } }} />
        ...
        {/* Book details */}
        <div className={sectionCls}>
          <h2 className="text-sm font-medium text-on-dark mb-5">Book Details</h2>
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
            ...
            <div>
              ...
              <input className={inputCls} value={form.genre} onChange={e => set("genre", e.target.value)} placeholder="Fiqh" />
            </div>
            <div>
              <label className={labelCls}>ISBN</label>
              <input className={inputCls} value={form.isbn} onChange={e => set("isbn", e.target.value)} placeholder="978-…" />
            </div>
            <div>
              <label className={labelCls}>Edition</label>
              <input className={inputCls} value={form.edition} onChange={e => set("edition", e.target.value)} placeholder="2nd" />
            </div>
            <div>
              <label className={labelCls}>Page Count</label>
              <input className={inputCls} type="number" min="1" value={form.pageCount} onChange={e => set("pageCount", e.target.value)} placeholder="450" />
            </div>
            <div className="sm:col-span-2">
            ...
            <div className="sm:col-span-2">
              ...
              <textarea rows={3} value={form.tableOfContents} onChange={e => set("tableOfContents", e.target.value)}
                className="w-full px-3 py-2 text-sm bg-surface-dark border border-white/10 rounded-md text-on-dark placeholder:text-on-dark-soft focus:outline-none focus:border-primary resize-none"
        ...
        {/* Images */}
        <div className={sectionCls}>
          <h2 className="text-sm font-medium text-on-dark mb-5">Cover & Images</h2>
          <input ref={fileRef} type="file" accept="image/*" multiple className="hidden" onChange={e => addFiles(e.target.files)} />
          {previews.length > 0 && (
            <div className="grid grid-cols-4 gap-3 mb-4">
              {previews.map((p, i) => (
                <div key={i} className="relative group aspect-[3/4] rounded-lg overflow-hidden bg-surface-dark-soft border border-white/10">
                  ...
                  {p.isCover && (
                    <span className="absolute bottom-1.5 left-1.5 text-[9px] font-medium bg-accent-amber text-white px-1.5 py-0.5 rounded uppercase tracking-wide">Cover</span>
                  )}
                </div>
              ))}
            </div>
