...
export default function EditBookPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    fetch(`/api/admin/products/${id}`).then(r => r.json()).then(({ product }) => {
      if (!product) { setError("Book not found."); setLoading(false); return; }
      setForm({
        title:           product.title ?? "",
        slug:            product.slug ?? "",
  ...
  function addFiles(files: FileList | null) {
    if (!files) return;
    const noCover = existingImages.every(i => !i.isCover) && newImages.every(i => !i.isCover);
    setNewImages(prev => [...prev, ...Array.from(files).map((file, i) => ({
  ...
  function setExistingCover(imgId: string) {
    setExistingImages(prev => prev.map(i => ({ ...i, isCover: i.id === imgId })));
  ...
  return (
    <div className="px-8 py-8 text-on-dark max-w-3xl">
      <div className="mb-6">
        <h1 className="text-2xl font-normal text-on-dark" style={{ fontFamily: "var(--font-display)" }}>Edit Book</h1>
        ...
        <div className={sectionCls}>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input className={inputCls} type="number" min="0" step="0.01" value={form.originalPrice} onChange={e => set("originalPrice", e.target.value)} />
            </div>
            <div>
          ...
          {(existingImages.length > 0 || newImages.length > 0) && (
            <div className="grid grid-cols-4 gap-3 mb-4">
              {existingImages.map(img => (
                <div key={img.id} className="relative group aspect-[3/4] rounded-lg overflow-hidden bg-surface-dark-soft border border-white/10">
                  <Image src={img.url} alt="" fill className="object-cover" sizes="120px" />
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                    <button type="button" onClick={() => setExistingCover(img.id)}
                  ...
                  {img.isCover && <span className="absolute bottom-1.5 left-1.5 text-[9px] font-medium bg-accent-amber text-white px-1.5 py-0.5 rounded uppercase tracking-wide">Cover</span>}
                </div>
              ))}
              {newImages.map((img, i) => (
                <div key={`new-${i}`} className="relative group aspect-[3/4] rounded-lg overflow-hidden bg-surface-dark-soft border border-white/10">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={img.url} alt="" className="w-full h-full object-cover" />
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                    <button type="button" onClick={() => setNewCover(i)}
                      className={`w-7 h-7 rounded-full flex items-center justify-center transition-colors ${img.isCover ? "bg-accent-amber" : "bg-white/20 hover:bg-accent-amber"}`}>
                      <Star size={12} className="text-white" />
                    </button>
