...
export default function NewProductPage() {
  ...
  const [form, setForm] = useState({
    title: "", slug: "", type: "book" as ProductType,
  ...
  function addVariant() { setVariants(v => [...v, { label: "", stock: "0", price: "" }]); }
  function updateVariant(i: number, k: keyof Variant, v: string) {
    setVariants(prev => prev.map((vv, n) => n === i ? { ...vv, [k]: v } : vv));
  }
  function removeVariant(i: number) { setVariants(prev => prev.filter((_, n) => n !== i)); }

  async function uploadToCloudinary(file: File, slug: string): Promise<{ url: string; cloudinaryId: string }> {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      for (const preview of previews) {
        const { url, cloudinaryId } = await uploadToCloudinary(preview.file, product.slug);
        await fetch(`/api/admin/products/${product.id}/images`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ url, cloudinaryId, isCover: preview.isCover }),
        });
      }
      ...
      for (const v of variants) {
        ...
        await fetch(`/api/admin/products/${product.id}/variants`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ label: v.label, stock: parseInt(v.stock) || 0, price: v.price ? parseFloat(v.price) : null }),
        });
      }
  ...
  return (
    <div className="px-8 py-8 text-on-dark max-w-3xl">
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-on-dark">Add Product</h1>
        <p className="text-sm text-on-dark-soft mt-0.5">New product will be visible on the storefront immediately.</p>
      </div>
      ...
      <form onSubmit={handleSubmit}>
          ...
          <div className={sectionCls}>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input className={inputCls} value={form.genre} onChange={e => set("genre", e.target.value)} placeholder="Islamic" />
              </div>
              <div>
        ...
        <div className={sectionCls}>
          ...
          <p className="text-xs text-on-dark-soft mt-2">Click the ★ on a preview to set the cover image.</p>
        </div>
        ...
        {!isBook && (
          <div className={sectionCls}>
            ...
            {variants.length === 0 ? (
              <p className="text-sm text-on-dark-soft">No variants — single product with base price and stock.</p>
            ) : (
                ...
                {variants.map((v, i) => (
                  <div key={i} className="grid grid-cols-[1fr_1fr_1fr_auto] gap-3 items-end">
                    <div>
                      ...
                      <input className={inputCls} value={v.label} onChange={e => updateVariant(i, "label", e.target.value)} placeholder="Small" />
                    </div>
                    <div>
                      {i === 0 && <label className={labelCls}>Stock</label>}
                      <input className={inputCls} type="number" min="0" value={v.stock} onChange={e => updateVariant(i, "stock", e.target.value)} placeholder="10" />
                    </div>
                    <div>
                      {i === 0 && <label className={labelCls}>Price (₹, optional)</label>}
                      <input className={inputCls} type="number" min="0" step="0.01" value={v.price} onChange={e => updateVariant(i, "price", e.target.value)} placeholder="—" />
                    </div>
