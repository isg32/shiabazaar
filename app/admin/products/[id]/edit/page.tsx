...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  function updateExistingVariant(variantId: string, k: "label" | "stock" | "price", v: string) {
    setExistingVariants(prev => prev.map(vv =>
      vv.id === variantId
        ? { ...vv, [k]: k === "stock" ? (parseInt(v) || 0) : k === "price" ? (parseFloat(v) || null) : v }
        : vv
    ));
  }

  function addNewVariant() { setNewVariants(v => [...v, { label: "", stock: "0", price: "" }]); }
  function updateNewVariant(i: number, k: keyof NewVariant, v: string) {
    setNewVariants(prev => prev.map((vv, n) => n === i ? { ...vv, [k]: v } : vv));
  }
  function removeNewVariant(i: number) { setNewVariants(prev => prev.filter((_, n) => n !== i)); }
          ...
          <div className={sectionCls}>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input className={inputCls} type="number" min="1" value={form.pageCount} onChange={e => set("pageCount", e.target.value)} />
              </div>
              <div className="sm:col-span-2">
        ...
        <div className={sectionCls}>
          ...
          {(existingImages.length > 0 || newImages.length > 0) && (
            <div className="grid grid-cols-4 gap-3 mb-4">
              ...
              {newImages.map((p, i) => (
                <div key={`new-${i}`} className="relative group aspect-[3/4] rounded-lg overflow-hidden bg-surface-dark-soft border border-primary/40">
                  ...
                  <span className="absolute top-1.5 right-1.5 text-[9px] font-medium bg-primary text-white px-1.5 py-0.5 rounded uppercase tracking-wide">New</span>
                </div>
              ))}
            </div>
          )}
          ...
          <p className="text-xs text-on-dark-soft mt-2">Click ★ to set cover. New images shown with blue border.</p>
        </div>
            ...
            {existingVariants.length === 0 && newVariants.length === 0 ? (
              ...
              <div className="flex flex-col gap-3">
                ...
                {existingVariants.map((v, i) => (
                  <div key={v.id} className="grid grid-cols-[1fr_1fr_1fr_auto] gap-3 items-end">
                    <div>
                      {i === 0 && <label className={labelCls}>Label</label>}
                      <input className={inputCls} value={v.label}
                    ...
                    <button type="button" onClick={() => removeExistingVariant(v.id)}
                ...
                {newVariants.map((v, i) => {
                  ...
                  return (
                    <div key={`new-${i}`} className="grid grid-cols-[1fr_1fr_1fr_auto] gap-3 items-end">
                      <div>
                        {offset === 0 && <label className={labelCls}>Label</label>}
                        <input className={inputCls} value={v.label}
                          onChange={e => updateNewVariant(i, "label", e.target.value)} placeholder="Small" />
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <input className={inputCls} type="number" min="0" step="0.01" value={v.price}
                          onChange={e => updateNewVariant(i, "price", e.target.value)} placeholder="—" />
                      </div>
                      <button type="button" onClick={() => removeNewVariant(i)}
